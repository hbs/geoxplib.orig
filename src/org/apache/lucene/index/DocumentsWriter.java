...
final class DocumentsWriter {
  ...
  boolean updateDocument(Document doc, Analyzer analyzer, Term delTerm)
    ...
    try {
      ...
      // GEOXP
      // 
      // Inform the GeoDataSegmentCache of the newly added
      // doc. It will return a docId to delete (the one of the
      // previous doc with that same GeoXP uuid).
      //
      ////////////////////////////////
      int previousDocId = GeoDataSegmentCache.track(doc, state.docState.docID);
      
      if (previousDocId >= 0) {
        // Delete previous version of this doc
        synchronized(this) {
  ...
  synchronized boolean applyDeletes(SegmentInfos infos) throws IOException {
    ...
    // GEOXP
    //
    // Mark current thread as deleting so we don't delete twice
    //
    GeoDataSegmentCache.setDeleting(true);
    ...
    GeoDataSegmentCache.setDeleting(false);
