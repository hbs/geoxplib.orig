...
public class HHCodeHelper {
  ...
  public static String bbox(double minLat, double minLon, double maxLat, double maxLon) {
    ...
    long ll = getHHCodeValue(minLat, minLon);
    long ur = getHHCodeValue(maxLat, maxLon);
    
    // Compute delta in latitude and longitude
    
    long[] llcoords = splitHHCode(ll);
    long[] urcoords = splitHHCode(ur);
    
    long deltaLat = urcoords[0] - llcoords[0];
    long deltaLon = urcoords[1] - llcoords[1];
    ...
    // Trim lower bits
    long startLat = llcoords[0] & (0xffffffff ^ ((1L << log) - 1));
    long startLon = llcoords[1] & (0xffffffff ^ ((1L << log) - 1));
    ...
    while (lat <= urcoords[0]) {
      lon = startLon;
      
      while (lon <= urcoords[1]) {
        long h = buildHHCode(lat, lon);
        String hs = toString(h, 32 - log);
